...
The entire backend lives in **`src-tauri/src/lib.rs`**. It runs four concurrent async tasks:

| Task | Description |
|------|-------------|
| UDP broadcaster | Sends `FASTPASTE:<hostname>:4567` to `255.255.255.255:4568` every 2s |
| WebSocket server | Listens on `0.0.0.0:4567`, handles per-client send/receive tasks |
| Clipboard poller | Polls clipboard every 800ms, pushes new text to history and WS broadcast channel |
| Setup | Registers global hotkey, system tray, close-to-tray behavior |

**State** is a single `Arc<Mutex<AppStateData>>` containing settings, history (max 100 items), IPs, and connected client IPs. It is persisted to `settings.json` next to the executable on every mutation.
...
- **`MainViewModel`** — holds `UiState` as `StateFlow`; starts/stops `ClipboardService` via intents; drives auto-connect on first UDP discovery
- **`ClipboardService`** — foreground service; owns `WebSocketClient`; listens to `ClipboardManager` for local changes and sends them over WS; applies incoming text to clipboard
- **`WebSocketClient`** — OkHttp-based WS with exponential backoff reconnect (max 5 attempts, cap 30s), 30s ping interval
- **`ServiceDiscovery`** — UDP listener on port 4568; stops when connected (battery optimization)
- **Room database** — `ClipboardEntry` table accessed via `ClipboardDao`; history stored locally up to 200 items
